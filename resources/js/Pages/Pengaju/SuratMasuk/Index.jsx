...
const PengajuDashboard = ({
    ...
    useEffect(() => {
        ...
        if (flash?.error) {
            Swal.fire({ title: "Error!", text: flash.error, icon: "error" });
        }
    }, [flash]);
    useEffect(() => {
    ...
    const handleAddNewClick = () => {
        ...
        const countThisYear = allSurat.filter(
            (surat) => new Date(surat.created_at).getFullYear() === currentYear
        ).length;
    ...
    const handleFormSubmit = (e) => {
        ...
        if (data.jenis_surat_id === "other" && !data.jenis_surat_manual) {
            Swal.fire({
                icon: "warning",
                title: "Jenis Surat Lainnya Kosong",
                text: "Mohon isi jenis surat lainnya!",
            });
            return;
        }
        const options = {
            ...
            onError: (errs) => {
                ...
                if (Object.keys(errs).length > 0) {
                    errorMessage = Object.values(errs)[0];
                }
                Swal.fire({
    ...
    const filteredAndSearchedSurat = useMemo(() => {
        ...
        if (searchQuery) {
            ...
            filtered = filtered.filter(
                (surat) =>
                    surat.perihal.toLowerCase().includes(lowerCaseQuery) ||
                    (surat.jenis_surat?.nama_jenis &&
                        surat.jenis_surat.nama_jenis
                            .toLowerCase()
                            .includes(lowerCaseQuery)) ||
                    (surat.jenis_surat_manual &&
                        surat.jenis_surat_manual
                            .toLowerCase()
                            .includes(lowerCaseQuery))
            );
        ...
        if (filterStatus && filterStatus !== "Semua Status") {
            filtered = filtered.filter((surat) => {
                const latestStatus = surat.tracking?.[0]?.status?.nama_status;
                return (
                    latestStatus &&
    ...
    const paginatedSurat = useMemo(() => {
        ...
        return filteredAndSearchedSurat.slice(
            startIndex,
            startIndex + itemsPerPage
        );
    }, [filteredAndSearchedSurat, currentPage, itemsPerPage]);
    const totalPages = Math.ceil(
        filteredAndSearchedSurat.length / itemsPerPage
    );
    ...
    const allUniqueStatuses = useMemo(() => {
        ...
        (suratMasuk || []).forEach((surat) => {
            if (surat.tracking?.[0]?.status?.nama_status) {
                statuses.add(surat.tracking[0].status.nama_status);
            }
        });
                        ...
                        <div className="flex items-center space-x-4">
                            <div className="relative" ref={dropdownRef}>
                                <button
                                    ...
                                    className="p-2 rounded-full text-gray-500 hover:text-gray-800 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-violet-500"
                                    aria-haspopup="true"
                                ...
                                {isDropdownOpen && (
                                    <div
                                        className="origin-top-right absolute right-0 mt-2 w-100 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-20"
                                        role="menu"
                                        ...
                                        <div className="py-1" role="none">
                                            <div className="px-6 py-4">
                                                ...
                                                <p className="text-sm font-medium text-gray-900 truncate flex items-center">
                                                    <span>{user.name}</span>
                                                    <span className="ml-2 px-2 py-0.5 bg-blue-100 text-blue-800 text-sm font-semibold rounded-full">
                                                        {
                                                            user.level_user
                                                                ?.nama_level
                                                        }
                            ...
                            <form onSubmit={handleLogout}>
                                <button
                                    type="submit"
                                    className="bg-red-500 hover:bg-red-600 text-white font-bold py-1 px-2 rounded-lg flex items-center"
                                >
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-6">
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setData((prevData) => ({
                                                    ...prevData,
                                                    jenis_surat_id: selectedId,
                                                    jenis_surat_manual:
                                                        selectedId !== "other"
                                                            ? ""
                                                            : prevData.jenis_surat_manual,
                                                }));
                                            }}
                                            className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-violet-500 focus:ring-violet-500 sm:text-sm"
                                            required
                                            ...
                                            {safeTujuan.map((userTujuan) => (
                                                ...
                                                >
                                                    {userTujuan
                                                        .jabatan_struktural
                                                        ?.nama_jabatan
                                                        ? `${userTujuan.jabatan_struktural.nama_jabatan} (${userTujuan.name})`
                                                        : userTujuan.name}
                                                </option>
                                            ))}
                                        </select>
                                    ...
                                    <div className="md:col-span-2">
                                        ...
                                        <textarea
                                            ...
                                            rows={4}
                                            className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-violet-500 focus:ring-violet-500 sm:text-sm"
                                            placeholder="Jelaskan secara singkat isi atau keterangan dari surat yang diajukan..."
                                    ...
                                    <div className="md:col-span-2">
                                        ...
                                        <input
                                            ...
                                            placeholder="Contoh: 123/UND/IV/2025"
                                            className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-violet-500 focus:ring-violet-500 sm:text-sm"
                                        />
                                        {errors.nomor_surat && (
                                            <div className="text-red-600 text-xs mt-1">
                                ...
                                <div className="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
                                    <div className="md:col-span-2 flex-1">
                                        <input
                                            id="search_query"
                                            type="text"
                                            placeholder="Cari Jenis Surat atau Perihal Surat"
                                            value={searchQuery}
                                            onChange={(e) => {
                                                            ...
                                                            <td className="px-6 py-4 flex items-center space-x-2">
                                                                ...
                                                                <button
                                                                    ...
                                                                    title={
                                                                        isEditable
                                                                            ? "Edit Surat"
                                                                            : "Surat tidak dapat diedit lagi karena sudah di verifikasi"
                                                                    }
                                                                >
                                                                    <svg
                                                                        xmlns="http://www.w3.org/2000/svg"
