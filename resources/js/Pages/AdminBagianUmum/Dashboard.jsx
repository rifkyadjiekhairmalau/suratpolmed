...
const TrackingItem = ({ item, isLast }) => {
    ...
    // Kondisi untuk menampilkan level user
    const shouldShowLevel = item.level_aksi_oleh &&
    ...
    const handleKembalikanClick = () => {
        if (!catatan.trim()) {
            Swal.fire({
                icon: "warning",
                title: "Input Kosong",
                text: "Alasan pengembalian/penolakan harus diisi!",
            });
            return;
        }
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col border-2 border-gray-200">
                <header className="p-6 border-b border-gray-200 sticky top-0 bg-white rounded-t-2xl z-10">
                    ...
                    <h2 className="text-2xl font-bold text-gray-800">
                        Proses Verifikasi: {surat.perihal}
                    </h2>
                </header>
                <div className="overflow-y-auto p-6 space-y-8">
                    <section>
                        ...
                        <dl>
                            ...
                            <DetailItem label="Tgl. Pengajuan">
                                {dayjs(surat.tgl_pengajuan).format(
                                    "DD MMMM YYYY"
                                )}
                            </DetailItem>
                            ...
                            <DetailItem label="File Surat">
                                ...
                                >
                                    <FileTextOutlined className="mr-2" /> Lihat
                                    File
                                </a>
                            </DetailItem>
                        </dl>
                    ...
                    <section>
                        ...
                        <div>
                            <label
                                htmlFor="tu_verification_notes"
                                className="block text-gray-700 text-sm font-medium mb-2"
                            >
                                Alasan Penolakan
                            </label>
                            <textarea
                                id="tu_verification_notes"
                                rows={3}
                                value={catatan}
                                onChange={(e) => setCatatan(e.target.value)}
                                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-400 text-sm"
                                placeholder="Wajib diisi jika surat dikembalikan/ditolak. Opsional jika diverifikasi..."
                            ></textarea>
                        </div>
                    </section>
                </div>
                <footer className="p-4 border-t border-gray-200 flex justify-end gap-4 sticky bottom-0 bg-white rounded-b-2xl z-10">
                    ...
                    <button
                        type="button"
                        className="px-6 py-2 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 transition duration-200 text-sm"
                    ...
                    <button
                        type="button"
                        className="px-6 py-2 bg-violet-700 text-white rounded-lg font-semibold hover:bg-violet-800 transition duration-200 shadow-sm text-sm"
...
export default function Dashboard({
    ...
    const transformSuratData = (surat) => ({
        ...
        status_terkini:
            surat.tracking.length > 0
                ...
                : "Belum ada status",
        ditujukan_kepada:
            surat.tujuan?.jabatanStruktural?.nama_jabatan ||
            surat.tujuan?.name ||
            "N/A",
        ...
        tracking_history: surat.tracking.map((t) => ({
            ...
            catatan: t.catatan || "",
            level_aksi_oleh: t.user?.level_user?.nama_level || null,
        })),
    });
    ...
    const filteredSurat = useMemo(() => {
        ...
        const STATUS_OPTIONS = Array.from(
            new Set(daftarSurat.map((s) => s.status_terkini))
        ).filter(Boolean);
        if (statusFilter !== "Semua Status") {
            filtered = filtered.filter(
                (surat) =>
                    surat.status_terkini.toLowerCase() ===
    ...
    const handleOpenModal = (surat, type) => {
    ...
    const handleCloseModal = () => {
        ...
        router.reload({
            only: ["suratUntukVerifikasi", "flash"],
            preserveScroll: true,
        });
    };
    ...
    const handleVerifikasi = (catatan) => {
        ...
        router.post(
            route("administrasi_umum.suratmasuk.verify", {
                suratMasuk: selectedSurat.id,
            }),
            { catatan: catatan },
            {
                onSuccess: () => {
                    ...
                    Swal.fire(
                        "Berhasil!",
                        "Surat berhasil diverifikasi dan diteruskan.",
                        "success"
                    );
                },
                onError: (errors) => {
                    console.error("Verifikasi Error:", errors);
                    Swal.fire(
                        "Gagal!",
                        errors.catatan ||
                            "Terjadi kesalahan saat memverifikasi surat.",
                        "error"
                    );
                },
                preserveScroll: true,
    ...
    const handleKembalikan = (alasan) => {
        if (!selectedSurat) return;
        router.post(
            route("administrasi_umum.suratmasuk.reject", {
                suratMasuk: selectedSurat.id,
            }),
            { catatan_penolakan: alasan },
            {
                ...
                onError: (errors) => {
                    console.error("Penolakan Error:", errors);
                    Swal.fire(
                        "Gagal!",
                        errors.catatan_penolakan ||
    ...
    const allUniqueStatuses = useMemo(() => {
        const statuses = new Set(
            daftarSurat.map((s) => s.status_terkini).filter(Boolean)
        );
        ...
        <div className="group bg-white p-6 rounded-2xl shadow-lg flex items-center gap-5 border border-transparent transition-all duration-200 cursor-pointer hover:border-violet-500 hover:shadow-xl hover:-translate-y-1 hover:bg-violet-50">
            <div className="w-14 h-14 flex items-center justify-center rounded-full bg-violet-100 group-hover:bg-violet-500 transition-all duration-200">
                {React.cloneElement(icon, {
                    className:
                        "h-7 w-7 text-violet-500 group-hover:text-white transition-all duration-200",
                })}
            </div>
            <div>
                <p className="text-black-500 text-sm font-semi-bold group-hover:text-violet-700 transition">
                    {title}
                </p>
                <p className="text-3xl font-extrabold text-gray-800 group-hover:text-violet-900 transition">
                    {value}
                </p>
            </div>
        </div>
            ...
            <main className="min-h-screen bg-gray-50 p-2 md:p-4">
                <div className="max-w-7xl mx-auto">
                    ...
                    <p className="text-gray-600 mb-4">
                        Kelola surat masuk, surat keluar dan pantau alur
                        disposisi.
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4 font-bold">
                        ...
                        />
                        <StatCard
                            title="Total Surat Menunggu Verifikasi"
                            value={suratBelumVerifikasiCount}
                            icon={<SuratMenungguIcon />}
                        />
                    </div>
                            ...
                            <div className="flex flex-col md:flex-row gap-4 w-full">
                                <div className="relative flex-grow">
                                    ...
                                    <input
                                        ...
                                        className=" w-1/2 pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-400 text-sm w-full"
                                    />
                                </div>
                            </div>
                        </div>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200 text-sm">
                                    {paginatedSurat.length > 0 ? (
                                        paginatedSurat.map((surat) => (
                                            ...
                                            >
                                                <td className="px-6 py-4 font-medium text-black-60">
                                                    {surat.no_agenda}
                                                </td>
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <div className="flex justify-center items-center gap-4">
                                                        ...
                                                        <button
                                                            ...
                                                            className="bg-violet-600 text-white px-3 py-1 rounded-md hover:bg-violet-700 font-semibold transition text-xs"
                                                        >
                                                            Proses
                                                        </button>
                                                    </div>
                    ...
                    {modalState === "verifikasi" && selectedSurat && (
                        <VerifikasiModal
                            surat={selectedSurat}
                    ...
                    {modalState === "detail" && selectedSurat && (
                        <DetailModal
                            surat={selectedSurat}
                            onClose={handleCloseModal}
                        />
