...
import { FaTachometerAlt, FaUsers, FaUserGraduate, FaBriefcase, FaSignOutAlt } from 'react-icons/fa';
        ...
        <aside className="sidebar min-h-screen bg-gradient-to-b from-violet-400 via-violet-500 to-violet-800 p-6 flex flex-col justify-between rounded-[22px] w-[260px]">
            <div>
                ...
                <nav>
                    ...
                    <Link href={route('admin.users.index')} className={route().current('admin.users.index') ? activeClass : inactiveClass}>
                        <FaUsers className={iconClass} />
                        Manajemen Pengguna
                    </Link>

                    {/* Link ke Manajemen Mahasiswa - Menggunakan FaUserGraduate */}
                    <Link href={route('admin.mahasiswa.index')} className={route().current('admin.mahasiswa.index') ? activeClass : inactiveClass}>
                        <FaUserGraduate className={iconClass} />
                        Manajemen Mahasiswa
                    </Link>

                    {/* Link ke Manajemen Pegawai - Menggunakan FaBriefcase */}
                    <Link href={route('admin.pegawai.index')} className={route().current('admin.pegawai.index') ? activeClass : inactiveClass}>
